...
internal sealed class HaWsConnection : IHaWsConnection
{
    ...
    async Task<T> PipeReadAsync<T>(CancellationToken cancellationToken)
    {
        try
        {
            var message = await JsonSerializer.DeserializeAsync<T>(pipe!.Reader.AsStream(), IHaWsConnection.JsonOptions,
    ...
    public async Task SendAsync<T>(T message, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var bytes = JsonSerializer.SerializeToUtf8Bytes(message, IHaWsConnection.JsonOptions);
